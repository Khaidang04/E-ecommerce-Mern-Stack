...
class W {
  ...
  update(e) {
    ...
    for (let idx = r; idx < t.count; idx++) {
      ...
      // Kiểm tra I.y < -t.maxY, không trừ radius
      if (I.y < -t.maxY) {
        I.y = -t.maxY;
...
class Z extends d {
  constructor(e, t = {}) {
    ...
    this.sphere0Material = new Y({
      envMap: n,
      emissive: i.emissive0,
      emissiveIntensity: 1,
      ...i.materialParams,
    });
  }
...
function createBallpit(e, t = {}) {
  ...
  i.onAfterResize = (e) => {
    ...
    s.config.maxY = e.wHeight - 9.5; // Trừ đi một khoảng để không rơi ra ngoài
    console.log("maxY:", s.config.maxY); // Kiểm tra giá trị maxY
  };
  return {
    three: i,
    get spheres() {
...
const Ballpit = ({
  ...
  useEffect(() => {
    ...
    if (!canvas) {
      console.error("Ballpit: Canvas not found");
      return;
    }
